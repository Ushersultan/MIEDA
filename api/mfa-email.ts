...
export default async function handler(req: any, res: any) {
  ...
  const { action, code, canal } = req.body ?? {};
  ...
  try {
    ...
    if (action === "send") {
      ...
      if (canal === "sms") {
        ...
        if (!sid || !tok || !from) {
          return res.status(500).json({ error: "SMS non configuré sur le serveur." });
        }
        const { data: prof } = await admin.from("profiles")
          .select("phone").eq("id", user.id).maybeSingle();
        telephone = (prof?.phone ?? "").replace(/[^+\d]/g, "");
        if (!/^\+\d{8,15}$/.test(telephone)) {
          return res.status(400).json({
            error: "Numéro invalide. Enregistrez-le au format international (+225..., +1...) dans votre profil.",
          });
        }
      }
      ...
      if (canal === "sms") {
        ...
        const corps = new URLSearchParams({
          To: telephone,
          From: from,
          Body: `MIEDA : votre code de vérification est ${nouveauCode}. Il expire dans ${VALIDITE_MIN} minutes.`,
        });
        ...
        if (!tw.ok) {
          ...
          console.error("Twilio:", detail.slice(0, 300));
          return res.status(502).json({ error: "Échec d'envoi du SMS. Réessayez ou utilisez l'email." });
        }
